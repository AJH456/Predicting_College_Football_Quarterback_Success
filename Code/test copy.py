...
'''def get_committed_school(soup):
    """Extract committed school from the timeline 'enrolls at' entry."""
    
    # Look for h4 tags in the timeline containing 'enrolls at'
    timeline = soup.find("section", class_="timeline")
    if timeline:
        for h4 in timeline.find_all("h4"):
            text = h4.get_text(strip=True)
            if "enrolls at" in text.lower():
                # Text is like "Bo Nix enrolls at Auburn Tigers"
                match = re.search(r'enrolls at (.+)', text, re.IGNORECASE)
                if match:
                    return match.group(1).strip()

    # Fallback: search entire page for 'enrolls at' pattern
    ...
    # Last resort: pull from meta keywords
    # Keywords format: "Bryce Young, Quarterback, Football, NFL, Carolina, Alabama, Mater Dei"
    meta = soup.find("meta", {"name": "keywords"})
    if meta:
        keywords = meta.get("content", "").split(",")
        # School is typically the last keyword
        if len(keywords) >= 2:
...
def get_player_urls(page):
    ...
    while True:
        if page_num == 1:
            ...
            url = (
                f"{BASE_URL}/season/2019-football/RecruitRankings/?InstitutionGroup=highschool&PositionGroup=QB/"
